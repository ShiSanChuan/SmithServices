...
void serial_port::receive_data(std::vector<char>& buffer)
{
	...
	// std::cout << "sizeofbuff : "<<sizeof(buffer) << std::endl;
	int read_bytes = read(_file_descriptor,rece, size);
	// std::cout << "sizeofread : "<< read_bytes << std::endl;
	buffer.clear();
	for(int i = 0; i < size; i++)
		buffer.push_back(rece[i]);
