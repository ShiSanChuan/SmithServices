...
bool RadioPort::init(std::string port, int speed, int flowCtrl, int databits, int stopbits, int parity)
{
    mPort=port;
    mSpeed = speed;
    mFlowCtrl = flowCtrl;
...
int RadioPort::Param_Set(int m_speed, int m_flow_ctrl, int m_databits, int m_stopbits, int m_parity)
{
    //设置串口数据帧格式
...
int RadioPort::Recv(unsigned char *rcv_buf,int data_len)
{
...
int RadioPort::Send(unsigned char *send_buf,int data_len)
{
    int len = 0;
...
bool RadioPort::Flush(void)
{
    tcflush(fd, TCIFLUSH);
    return true;
}

void RadioPort::ShowParam(void)
{
    cout << "fd        = " << fd << endl
